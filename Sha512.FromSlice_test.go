...
func TestSha512_FromSlice(t *testing.T) {
	...
	t.Run("Test FromSlice() Truncating overflow", func(t *testing.T) {
		...
		hash.FromSlice(actual)
		if !bytes.Equal(actual[:Sha512Length], expected[:]) {
			t.Fatalf("Mismatch\n"+
				"Actual:  %v\n"+
				"Expected:%v\n",
				actual, expected)
	...
	t.Run("Test FromSlice() Empty Slice", func(t *testing.T) {
		...
		var expected [Sha512Length]byte //leave it empty
		hash.FromSlice([]byte{})
		if !bytes.Equal(actual[:], expected[:]) {
			t.Fatalf("Mismatch\n"+
				"Actual:%v\n"+
